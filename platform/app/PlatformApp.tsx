...
function Trends({ reloadApp, notify }: { reloadApp: () => Promise<void>; notify: (message: string) => void }) {
  ...
  useEffect(() => {
    if (data && !targetAccountId) setTargetAccountId(data.settings?.target_account_id || data.accounts[0]?.id || "");
  ...
  async function startScan() {
    ...
    try {
      ...
      notify(`${analysis.core_samples ?? 0} 条核心样本，已自动整理出 ${analysis.created} 个可领取选题${analysis.warning ? `；${analysis.warning}` : ""}${scanWarnings.length ? `。补充说明：${scanWarnings.join("；")}` : ""}`);
