...
async function runtimeAI(path: "settings" | "test", options?: RequestInit) {
  ...
  try {
    response = await managerFetch(`/ai/${path}`, options);
  } catch {
    throw new Error("本机 AI 运行管理器未启动");
  }
...
export async function GET(request: Request) {
  ...
  const [accounts, topics, claims, logs, users, aiSettings] = await Promise.all(
    [
      db
        .prepare(
          "SELECT * FROM accounts WHERE is_demo=0 ORDER BY updated_at, rowid",
        )
        .all<DbRow>(),
      db
        .prepare(
          `SELECT t.*,u.name AS creator_name,i.brief,i.target_audience,i.pain_point,i.hook_points,i.content_structure,
      ...
      WHERE t.archived_at IS NULL ORDER BY t.created_at DESC`,
        )
        .all<DbRow>(),
      db
        .prepare(
          `SELECT c.*,t.title AS topic_title,t.platform AS topic_platform,a.name AS account_name,a.color AS account_color,a.platform AS account_platform,u.name AS owner_name,
      ...
      WHERE a.is_demo=0 ORDER BY c.updated_at DESC`,
        )
        .all<DbRow>(),
      db
        .prepare(
          "SELECT l.*,u.name AS actor_name FROM audit_logs l JOIN users u ON u.id=l.actor_id ORDER BY l.created_at DESC LIMIT 50",
        )
        .all<DbRow>(),
      db
        .prepare(
          "SELECT id,name,username,roles,status,created_at FROM users WHERE status='active' ORDER BY created_at",
        )
        .all<DbRow>(),
      runtimeAI("settings").catch(() => ({
        configured: false,
  ...
  return Response.json({
    ...
    topics: topics.results.map((topic) => ({
      ...
      content_structure: topic.content_structure
        ? JSON.parse(String(topic.content_structure))
        : [],
      source_feed_ids: topic.source_feed_ids
    ...
    claims: claims.results.map((claim) => ({
      ...
      publish_recoverable:
        ...
        Date.now() - Date.parse(String(claim.updated_at)) >= 8 * 60 * 1000,
      ...
      publish_snapshot: claim.snapshot
        ? JSON.parse(String(claim.snapshot))
        : null,
      wechat_style: JSON.parse(String(claim.wechat_style || "{}")),
    })),
    logs: logs.results,
...
export async function POST(request: Request) {
  ...
  if (action === "save_wechat_layout" || action === "preview_wechat_layout") {
    ...
    const claim = await db
      .prepare(
        "SELECT owner_id,status,content_type,body,snapshot,wechat_theme,wechat_style FROM claims WHERE id=?",
      )
      .bind(id)
      .first<Record<string, unknown>>();
    if (!claim || claim.content_type !== "wechat_article")
      return Response.json({ error: "公众号文章不存在" }, { status: 404 });
    const canPreview = can(user, [
      ...roleGroups.operate,
      ...roleGroups.review,
      "publisher",
    ]);
    if (!canPreview) return forbidden("你没有查看公众号排版的权限");
    ...
    const frozen = claim.snapshot
      ? (JSON.parse(String(claim.snapshot)) as Record<string, unknown>)
      : null;
    const theme = String(
      data.theme || frozen?.wechat_theme || claim.wechat_theme || "default",
    );
    if (!themes.has(theme))
      return Response.json(
        { error: "不支持的公众号排版主题" },
    ...
    const sourceStyle = (data.style ||
      frozen?.wechat_style ||
      JSON.parse(String(claim.wechat_style || "{}"))) as Record<
      string,
      unknown
    >;
    const style = {
    ...
    if (action === "save_wechat_layout") {
      if (
        !can(user, roleGroups.operate) ||
        !["writing", "revision"].includes(String(claim.status))
      )
        return Response.json(
          { error: "只能在创作或修改阶段调整排版" },
          { status: 409 },
        );
      await db
        .prepare(
          "UPDATE claims SET wechat_theme=?,wechat_style=?,updated_at=? WHERE id=?",
        )
        .bind(theme, JSON.stringify(style), now, id)
        .run();
      await audit(user.id, "设置公众号审核排版", "claim", id, theme);
    }
    const markdown = String(frozen?.body || claim.body || "").trim();
    if (!markdown)
      return Response.json({ error: "请先完成并保存正文" }, { status: 409 });
    ...
    return Response.json({ ok: true, html: payload.html || "", theme, style });
  }
  ...
  if (action === "save_ai_settings" || action === "test_ai_settings") {
    ...
    if (!roles.includes("admin"))
      return Response.json(
        { error: "只有管理员可以修改 AI 配置" },
        { status: 403 },
      );
    try {
      if (action === "test_ai_settings")
        return Response.json(
          await runtimeAI("test", {
            method: "POST",
            headers: { "content-type": "application/json" },
            body: JSON.stringify({
      ...
      await audit(
        ...
        "settings",
        "ai",
        `${payload.baseUrl} / ${payload.model}`,
      );
      return Response.json(payload);
    } catch (error) {
      return Response.json(
        { error: error instanceof Error ? error.message : "AI 配置操作失败" },
        { status: 502 },
      );
    }
  }
  ...
  if (action === "create_account") {
    ...
    if (!name)
      return Response.json({ error: "请填写账号备注名称" }, { status: 400 });
    const colors = ["#6846d6", "#178f78", "#d5723b", "#3d79c5", "#c9465d"];
    const id = crypto.randomUUID();
    const count = await db
      .prepare(
        "SELECT COUNT(*) AS total FROM accounts WHERE is_demo=0 AND platform=?",
      )
      .bind(platform)
      .first<{ total: number }>();
    const label =
    ...
    if ((count?.total ?? 0) >= 5)
      return Response.json(
        { error: `当前版本最多管理5个${label}账号` },
        { status: 409 },
      );
    await db
      .prepare(
        "INSERT INTO accounts (id,name,status,color,is_demo,platform,updated_at) VALUES (?,?,'login_expired',?,0,?,?)",
      )
      .bind(
        id,
        name,
        colors[(count?.total ?? 0) % colors.length],
        platform,
        now,
      )
      .run();
  ...
  if (action === "update_account_strategy") {
    ...
    const account = await db
      .prepare("SELECT id,name FROM accounts WHERE id=? AND is_demo=0")
      .bind(accountId)
    ...
    if (pillars.length < 2)
      return Response.json({ error: "请至少填写2个内容支柱" }, { status: 400 });
    ...
    await db
      .prepare(
        "UPDATE accounts SET persona=?,audience=?,content_pillars=?,strategy_keywords=?,excluded_topics=?,updated_at=? WHERE id=?",
      )
      .bind(
        persona,
        audience,
    ...
    await audit(
      ...
      "account",
      account.id,
      `${account.name} / ${pillars.join("、")} / ${keywords.join("、")}`,
    );
  ...
  if (action === "create_user") {
    ...
    const name = String(data.name ?? "").trim();
    if (!name || !/^[a-z0-9_-]{3,24}$/.test(username) || password.length < 8)
      return Response.json(
  ...
  if (action === "remove_user") {
    ...
    if (!targetId)
      return Response.json({ error: "请选择要删除的成员" }, { status: 400 });
    if (targetId === user.id)
      return Response.json(
        { error: "管理员不能删除自己的当前账号" },
        { status: 409 },
      );
    ...
    if (!target || target.status !== "active")
      return Response.json({ error: "成员不存在或已经删除" }, { status: 404 });
    await db.batch([
      db
  ...
  if (action === "create_topic") {
    if (!can(user, roleGroups.operate))
      return forbidden("只有管理员或内容运营可以创建选题");
    const title = String(data.title ?? "").trim();
    if (!title)
      return Response.json({ error: "请输入选题标题" }, { status: 400 });
    const platform = isPlatform(data.platform) ? data.platform : "xiaohongshu";
    const id = crypto.randomUUID();
    await db
      .prepare(
        "INSERT INTO topics (id,title,source_url,relevance,status,platform,source_type,source_external_id,created_by,created_at) VALUES (?,?,?,?,?,?,?,?,?,?)",
      )
      .bind(
        id,
        title,
        String(data.source_url ?? ""),
        String(data.relevance ?? "中"),
        "unclaimed",
        platform,
        String(data.source_type ?? "manual").slice(0, 40),
        String(data.source_external_id ?? "").slice(0, 255) || null,
        user.id,
        now,
      )
      .run();
  ...
  if (action === "archive_topics_bulk") {
    if (!can(user, roleGroups.operate))
      return forbidden("只有管理员或内容运营可以删除选题");
    const topicIds = [
      ...new Set(
    ...
    if (!topicIds.length)
      return Response.json({ error: "请选择要删除的选题" }, { status: 400 });
    const placeholders = topicIds.map(() => "?").join(",");
    const activeTopics = await db
      .prepare(
        `SELECT id FROM topics WHERE archived_at IS NULL AND id IN (${placeholders})`,
      )
      .bind(...topicIds)
      .all<{ id: string }>();
    if (!activeTopics.results.length)
      return Response.json(
        { error: "所选选题不存在或已经删除" },
        { status: 404 },
      );
    await db.batch(
      activeTopics.results.map((topic) =>
        db
          .prepare(
    ...
    await audit(
      ...
      "topic",
      "bulk",
      `${activeTopics.results.length} 个选题（历史内容任务保留）`,
    );
  ...
  if (action === "claim_topic") {
    if (!can(user, roleGroups.operate))
      return forbidden("只有管理员或内容运营可以认领选题");
    const topicId = String(data.topic_id ?? "");
    const accountId = String(data.account_id ?? "");
    if (!topicId || !accountId)
      return Response.json({ error: "请选择选题和账号" }, { status: 400 });
    const source = await db
      .prepare(
        `SELECT t.platform,s.feed_id,s.processing_status,s.detail_text FROM topics t
    ...
    if (
      source?.feed_id &&
      (source.processing_status !== "success" || !source.detail_text)
    ) {
      return Response.json(
        { error: "这个自动选题的来源正文尚未验证，暂时不能认领创作" },
        { status: 409 },
      );
    }
    const account = await db
      .prepare("SELECT id,platform FROM accounts WHERE id=? AND is_demo=0")
      .bind(accountId)
    ...
    try {
      await db.batch([
        db
          .prepare(
            "INSERT INTO claims (id,topic_id,account_id,owner_id,angle,status,content_type,created_at,updated_at) VALUES (?,?,?,?,?,'writing',?,?,?)",
          )
          .bind(
            id,
            topicId,
            ...
            String(data.angle ?? ""),
            contentTypeForPlatform(
              account.platform as "xiaohongshu" | "zhihu" | "wechat",
            ),
            now,
            now,
          ),
        db
          .prepare("UPDATE topics SET status='claimed' WHERE id=?")
          .bind(topicId),
      ]);
    } catch (error) {
  ...
  if (action === "save_draft") {
    if (!can(user, roleGroups.operate))
      return forbidden("只有管理员或内容运营可以保存草稿");
    const id = String(data.id ?? "");
    const claim = await db
    ...
    await db
      .prepare(
        "UPDATE claims SET title=?,body=?,tags=?,status=CASE WHEN status='revision' THEN 'writing' ELSE status END,updated_at=? WHERE id=?",
      )
      .bind(
  ...
  if (action === "upload_review_images") {
    if (!can(user, roleGroups.operate))
      return forbidden("只有管理员或内容运营可以上传审核图片");
    const id = String(data.id ?? "");
    const claim = await db
      .prepare("SELECT id,status FROM claims WHERE id=?")
      .bind(id)
    ...
    if (!files.length || files.some((file) => !file.data || !file.type))
      return Response.json({ error: "请选择1-9张有效图片" }, { status: 400 });
    const response = await managerFetch("/publish-assets", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ claimId: id, files }),
    }).catch(() => null);
    if (!response)
      return Response.json(
        { error: "本机图片保存服务未启动" },
    ...
    if (!response.ok || !payload.paths?.length)
      return Response.json(
        { error: payload.error || "图片保存失败" },
        { status: 502 },
      );
    const result = await db
      .prepare(
        "UPDATE claims SET publish_images=?,updated_at=? WHERE id=? AND status IN ('writing','revision')",
      )
      .bind(JSON.stringify(payload.paths), now, id)
      .run();
    if (!result.meta.changes)
      return Response.json(
        { error: "内容状态已变化，无法更换图片" },
        { status: 409 },
      );
    await audit(
      user.id,
  ...
  if (action === "submit_review") {
    if (!can(user, roleGroups.operate))
      return forbidden("只有管理员或内容运营可以提交审核");
    const id = String(data.id ?? "");
    const claim = await db
      .prepare(
        "SELECT owner_id,title,body,publish_images,status,content_type FROM claims WHERE id=?",
      )
      .bind(id)
      .first<{
        owner_id: string;
    ...
    if (!["writing", "revision"].includes(claim.status))
      return Response.json(
        { error: "只有创作中或待修改的内容可以提交审核" },
        { status: 409 },
      );
    if (!claim.title.trim() || !claim.body.trim())
      return Response.json(
        { error: "标题和正文填写完整后才能提交" },
        { status: 400 },
      );
    const images = JSON.parse(claim.publish_images || "[]") as string[];
    if (requiresReviewImages(claim.content_type) && !images.length)
      return Response.json(
        { error: "请先上传至少一张最终图片，再提交图文审核" },
        { status: 400 },
      );
    await db
      .prepare(
        "UPDATE claims SET status='review',review_comment='',updated_at=? WHERE id=? AND status IN ('writing','revision')",
      )
      .bind(now, id)
      .run();
  ...
  if (action === "review") {
    ...
    if (result === "revision" && !comment)
      return Response.json({ error: "退回时必须填写原因" }, { status: 400 });
    const claim = await db
      .prepare(
        "SELECT title,body,tags,creative_json,version_number,publish_images,status,content_type,wechat_theme,wechat_style FROM claims WHERE id=?",
      )
      .bind(id)
      .first<Record<string, unknown>>();
    if (!claim || claim.status !== "review")
      return Response.json(
        { error: "内容当前不在待审核状态" },
        { status: 409 },
      );
    const reviewImages = JSON.parse(
      String(claim.publish_images || "[]"),
    ) as string[];
    if (
      result === "approved" &&
      requiresReviewImages(claim.content_type) &&
      !reviewImages.length
    )
      return Response.json(
        { error: "缺少最终图片，不能通过图文审核" },
        { status: 400 },
      );
    const snapshot =
      result === "approved"
        ? JSON.stringify({
            ...
            approved_at: now,
            wechat_theme: claim.wechat_theme,
            wechat_style: JSON.parse(String(claim.wechat_style || "{}")),
          })
        : null;
    await db
      .prepare(
        "UPDATE claims SET status=?,review_comment=?,reviewer_id=?,snapshot=?,updated_at=? WHERE id=?",
      )
      .bind(result, comment, user.id, snapshot, now, id)
      .run();
    await audit(
      user.id,
      result === "approved" ? "审核通过" : "审核退回",
      "claim",
      id,
      comment,
    );
  ...
  if (action === "queue_publish") {
    ...
    await db
      .prepare("UPDATE claims SET status='queued',updated_at=? WHERE id=?")
      .bind(now, id)
      .run();
    await audit(
      user.id,
