...
const appBundle = resolve(outputRoot, "多平台内容运营.app");
const legacyAppBundle = resolve(outputRoot, "红薯台.app");
...
const dmgPath = resolve(
  outputRoot,
  `多平台内容运营-${version}-macOS-arm64-unsigned.dmg`,
);
  ...
  for (const entry of readdirSync(root, { withFileTypes: true })) {
    ...
    if (entry.isSymbolicLink()) {
      ...
      if (isAbsolute(target))
        throw new Error(`安装包包含绝对符号链接：${path} -> ${target}`);
      if (!existsSync(resolve(dirname(path), target)))
        throw new Error(`安装包包含失效符号链接：${path} -> ${target}`);
...
function makeCopiedSymlinksPortable(root, originalRoot, copiedRoot = root) {
  for (const entry of readdirSync(root, { withFileTypes: true })) {
    ...
    if (entry.isSymbolicLink()) {
      ...
      if (originalRelativeTarget.startsWith(".."))
        throw new Error(
          `Chromium 包含指向包外的绝对符号链接：${path} -> ${target}`,
        );
...
function assertNoCustomerData(root) {
  ...
  for (const path of forbiddenDirectories) {
    if (existsSync(resolve(root, path)))
      throw new Error(`安装包包含本地业务数据目录：${path}`);
  }
    ...
    for (const entry of readdirSync(directory, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) {
        if (/^(?:backups|migration-rollback(?:-|$))/.test(entry.name))
          throw new Error(`安装包包含备份或回滚数据：${packagedPath}`);
        walk(path);
      } else if (
...
const currentReleaseNames = new Set([
  basename(dmgPath),
  basename(`${dmgPath}.sha256`),
]);
for (const entry of readdirSync(outputRoot)) {
  if (
    !/^(?:红薯台|多平台内容运营)-.+-macOS-arm64-unsigned\.dmg(?:\.sha256)?$/.test(
      entry,
    ) ||
  ...
  const safeDestination = existsSync(destination)
...
cpSync(
  resolve(projectRoot, "runtime/bin"),
  resolve(packagedApp, "runtime/bin"),
  { recursive: true },
);
...
const packagedChromium = resolve(
  packagedApp,
  "runtime/bin/chromium/Chromium.app",
);
...
copyFileSync(
  ...
  resolve(resources, "runtime/LICENSE.Node.txt"),
);
writeFileSync(
  resolve(resources, "runtime/NODE_VERSION.txt"),
...
run("/usr/bin/clang", [
  ...
  "-o",
  resolve(contents, "MacOS/HongShuTai"),
]);
...
try {
  ...
  writeFileSync(
    resolve(dmgStage, "安装说明.txt"),
    [
      "多平台内容运营 Local 未签名测试版",
      "",
      "1. 将“多平台内容运营.app”拖入 Applications（应用程序）。",
      "2. 首次启动请右键“多平台内容运营.app”并选择“打开”，再确认打开。",
      "3. 首次启动先选择“独立使用”“创建团队主机”或“加入已有团队”。",
      "4. 菜单栏的应用图标可以重新打开平台、查看数据目录或退出平台。",
      "5. 多人协作请在菜单栏选择“局域网团队设置…”，由一台 Mac 作为主机，其他 Mac 填写主机地址和团队连接码。",
      "",
      "本应用未使用 Apple Developer ID 签名或公证。不要关闭 Gatekeeper，也不要执行陌生终端命令。",
      "为兼容旧版本，数据库、Cookie、AI Key 和素材仍保存在 ~/Library/Application Support/红薯台。",
    ].join("\n"),
  );
  run("/usr/bin/hdiutil", [
    "create",
