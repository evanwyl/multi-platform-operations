...
if (!sources.some(([name]) => name === "database")) throw new Error("没有找到本地数据库，请先启动并初始化平台");

mkdirSync(backupRoot, { recursive: true, mode: 0o700 });
const staging = mkdtempSync(join(tmpdir(), "hongshutai-backup-"));
try {
  for (const [name, source] of sources) cpSync(source, join(staging, name), { recursive: true, preserveTimestamps: true });
  ...
  const result = spawnSync("tar", ["-czf", destination, "-C", staging, "."], { stdio: "inherit" });
