...
const archiveArg = process.argv.find((value) => !value.startsWith("--") && value !== process.argv[0] && value !== process.argv[1]);
...
if (entries.some((entry) => entry.startsWith("/") || entry.split("/").includes(".."))) throw new Error("备份包包含不安全路径，已拒绝恢复");

const staging = mkdtempSync(join(tmpdir(), "hongshutai-restore-"));
const rollbackRoot = join(dataRoot, `restore-rollback-${new Date().toISOString().replace(/[:.]/g, "-")}`);
try {
  ...
  if (manifest.format !== 1 || manifest.product !== "红薯台" || !Array.isArray(manifest.contents)) throw new Error("这不是受支持的红薯台备份包");
  const targets = [
  ...
  if (!targets.some(([name]) => name === "database")) throw new Error("备份包不包含数据库");
  ...
  for (const [name, target] of targets) {
    ...
    cpSync(join(staging, name), target, { recursive: true, preserveTimestamps: true });
  }
  process.stdout.write(`恢复完成。恢复前的数据保存在：${rollbackRoot}\n确认平台正常后可手动删除该回滚目录。\n`);
} catch (error) {
  if (existsSync(rollbackRoot)) process.stderr.write(`恢复未完成，原数据副本位于：${rollbackRoot}\n`);
  throw error;
} finally {
