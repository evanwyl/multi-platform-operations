...
import {
  publishZhihuArticle,
  saveZhihuCredentials,
  zhihuAuthStatus,
} from "./platforms/zhihu-openapi.mjs";
import {
  closeZhihuBrowsers,
  openZhihuBrowserLogin,
  publishZhihuArticleBrowser,
  zhihuBrowserAuthStatus,
} from "./platforms/zhihu-browser.mjs";
import {
  publishWechatDraft,
  saveWechatCredentials,
  testWechatCredentials,
  wechatAuthStatus,
} from "./platforms/wechat-openapi.mjs";
import {
  renderWechatMarkdown,
  wechatThemes,
} from "./platforms/wechat-markdown.mjs";
...
const binary =
  ...
  (existsSync(bundledXhsBinary) ? bundledXhsBinary : legacyXhsBinary);
const promptFiles = {
  ...
  wechatContent: resolve(appRoot, "runtime/prompts/wechat-content-system.md"),
};
const analysisSchemas = {
  ...
  "zhihu-article-draft": resolve(
    appRoot,
    "runtime/zhihu-article-draft.schema.json",
  ),
  "wechat-article-draft": resolve(
    appRoot,
    "runtime/zhihu-article-draft.schema.json",
  ),
  "wechat-cover-prompt": resolve(
    appRoot,
    "runtime/wechat-cover-prompt.schema.json",
  ),
};
...
if (managerToken.length < 32) {
  process.stderr.write(
    "本机运行管理器缺少安全访问令牌。请使用 npm run dev 或 npm start 启动平台。\n",
  );
  process.exit(1);
}
...
if (!existsSync(binary)) {
  process.stderr.write(
    `找不到小红书 MCP。请设置 XHS_MCP_BINARY，或把可执行文件放到：${bundledXhsBinary}\n`,
  );
  process.exit(1);
}
...
function readAISettings() {
  let saved = {};
  try {
    saved = JSON.parse(readFileSync(aiConfigPath, "utf8"));
  } catch {
    void 0;
  }
  return {
    baseUrl: String(
      process.env.AI_BASE_URL || saved.baseUrl || defaultAISettings.baseUrl,
    )
      .trim()
      .replace(/\/+$/, ""),
    model: String(
      process.env.AI_MODEL || saved.model || defaultAISettings.model,
    ).trim(),
...
function publicAISettings() {
  ...
  return {
    ...
    keySource: process.env.AI_API_KEY
      ? "environment"
      : settings.apiKey
        ? "local"
        : "none",
...
function validateBaseUrl(value) {
  ...
  try {
    ...
    throw Object.assign(new Error("AI API 地址格式不正确"), { status: 400 });
  }
  const local = ["localhost", "127.0.0.1", "::1"].includes(url.hostname);
  if (
    url.username ||
    url.password ||
    !["https:", ...(local ? ["http:"] : [])].includes(url.protocol)
  ) {
    throw Object.assign(
      new Error("远程 AI API 必须使用 HTTPS；本机服务可以使用 HTTP"),
      { status: 400 },
    );
  }
...
function saveAISettings(payload) {
  if (process.env.AI_API_KEY)
    throw Object.assign(new Error("AI 配置由环境变量管理，不能从页面修改"), {
      status: 409,
    });
  ...
  if (!next.model)
    throw Object.assign(new Error("请填写 AI 模型名称"), { status: 400 });
  if (!next.apiKey)
    throw Object.assign(new Error("请填写 AI API Key"), { status: 400 });
...
function systemPrompt(kind) {
  if (kind === "wechat-cover-prompt") {
    if (!existsSync(promptFiles.wechatContent)) {
      throw Object.assign(new Error("平台内置公众号封面策划规则缺失"), {
        status: 503,
      });
    }
    return `${readFileSync(promptFiles.wechatContent, "utf8")}\n\n本次只生成一条公众号封面图提示词，不改写文章，不输出正文配图。`;
  }
  if (kind === "wechat-article-draft") {
    if (
      !existsSync(promptFiles.wechatContent) ||
      !existsSync(promptFiles.humanizer)
    ) {
      throw Object.assign(
        new Error("平台内置公众号编辑或 Humanizer 规则缺失"),
        { status: 503 },
      );
    }
  ...
  if (kind === "zhihu-article-draft") {
    if (
      !existsSync(promptFiles.zhihuContent) ||
      !existsSync(promptFiles.humanizer)
    ) {
      throw Object.assign(new Error("平台内置知乎专家或 Humanizer 规则缺失"), {
        status: 503,
      });
    }
  ...
  if (kind !== "content-draft") {
    if (!existsSync(promptFiles.research))
      throw Object.assign(new Error("平台内置 AI 规则缺失"), { status: 503 });
    return readFileSync(promptFiles.research, "utf8");
  }
  if (!existsSync(promptFiles.content) || !existsSync(promptFiles.humanizer)) {
    throw Object.assign(new Error("平台内置小红书专家或 Humanizer 规则缺失"), {
      status: 503,
    });
  }
...
async function cacheTrendCover(feedId, sourceUrl) {
  ...
  for (const extension of ["jpg", "png", "webp", "avif"]) {
    ...
    if (existsSync(join(trendCoverRoot, filename)))
  ...
  if (!remote.ok || !allowedXhsImageUrl(remote.url))
    throw Object.assign(new Error(`下载头图失败（${remote.status}）`), {
      status: 502,
    });
  const mime = String(remote.headers.get("content-type") || "")
    .split(";")[0]
    .trim()
    .toLowerCase();
  const extension =
      ...
      : mime === "image/webp"
        ...
        : mime === "image/avif"
          ...
          : ["image/jpeg", "image/jpg"].includes(mime)
            ...
            : "";
  if (!extension)
    throw Object.assign(new Error("小红书返回的头图格式不支持"), {
      status: 502,
    });
  const buffer = Buffer.from(await remote.arrayBuffer());
  if (!buffer.length || buffer.length > 12 * 1024 * 1024)
    throw Object.assign(new Error("头图为空或超过12MB"), { status: 502 });
  const filename = `${feedId}.${extension}`;
...
async function requestCompletion(settings, requestBody, signal) {
  ...
  try {
    ...
    if (signal.aborted)
      throw Object.assign(new Error("AI 任务已停止"), { status: 499 });
    throw Object.assign(
      new Error(
        `无法连接 AI API：${error instanceof Error ? error.message : "网络错误"}`,
      ),
      { status: 502 },
    );
  }
...
async function runAI(kind, prompt) {
  ...
  if (!schema || !existsSync(schema))
    throw Object.assign(new Error("AI 结构化输出格式未配置"), { status: 400 });
  const settings = readAISettings();
  if (!settings.apiKey)
    throw Object.assign(
      new Error("尚未配置 AI API Key，请让管理员前往系统设置完成配置"),
      { status: 503 },
    );
  if (analysisBusy)
    throw Object.assign(new Error("AI 正在处理上一项任务，请稍后重试"), {
      status: 409,
    });
  analysisBusy = true;
  ...
  try {
    ...
    try {
      ...
      const fallbackEligible =
        ...
        /json.?schema|response.?format|structured|unsupported/i.test(
          error.message || "",
        );
    ...
    if (!text)
      throw Object.assign(
        new Error(
          payload?.choices?.[0]?.message?.refusal || "AI 没有返回可识别的内容",
        ),
        { status: 502 },
      );
    try {
      ...
      throw Object.assign(
        new Error("AI 返回的结构化结果无法识别，请更换支持 JSON 输出的模型"),
        { status: 502 },
      );
    }
  } finally {
...
async function testAIConnection(overrides = {}) {
  ...
  try {
    ...
    if (!completionText(payload).trim())
      throw Object.assign(new Error("AI 服务已响应，但没有返回文本"), {
        status: 502,
      });
      ...
      const response = await fetch(endpoint, {
        ...
        body: JSON.stringify({
          ...
          params: {
            ...
            capabilities: {},
            clientInfo: { name: "hongshutai-runtime", version: "0.1.0" },
          },
        }),
      });
...
async function startProcess(accountId, slot) {
  ...
  for (const directory of [accountRoot, config, logs])
  ...
  const child = spawn(
    binary,
    ["-port", `${HOST}:${slot.port}`, `-headless=${!HEADED_BROWSER}`],
    {
      cwd: accountRoot,
      env: {
        ...process.env,
  ...
  try {
    await ready(slot.port, child);
  } catch (error) {
    stop(slot, "启动失败");
    throw error;
  }
  process.stdout.write(
    `账号 ${accountId.slice(0, 8)} 已进入${slot.purpose === "verification" ? "可见验证" : "有头采集"}槽位 ${slot.port}\n`,
  );
  return slot;
}
...
async function acquire(accountId, purpose = "worker") {
  ...
  const free =
    existing || conflicting
      ? null
      : slots.find(
          (slot) => slot.purpose === purpose && !slot.child && !slot.stopping,
        );
  ...
  return new Promise((resolvePromise, rejectPromise) => {
    ...
    waiter.timer = setTimeout(() => {
      ...
      rejectPromise(
        Object.assign(new Error("等待小红书运行槽位超过10分钟，任务已取消"), {
          status: 409,
        }),
      );
    }, QUEUE_TIMEOUT_MS);
...
async function drainQueue() {
  ...
  try {
    while (acquireQueue.length) {
      const sameIndex = acquireQueue.findIndex((waiter) =>
        slots.some(
          (slot) =>
            slot.purpose === waiter.purpose &&
            slot.accountId === waiter.accountId &&
            slot.child?.exitCode === null &&
            !slot.leased &&
            !slot.stopping,
        ),
      ...
      if (sameIndex >= 0) {
        ...
        const slot = slots.find(
          (candidate) =>
            candidate.purpose === waiter.purpose &&
            candidate.accountId === waiter.accountId &&
      ...
      const eligibleIndex = acquireQueue.findIndex(
        (waiter) =>
          !slots.some(
            (slot) =>
              slot.accountId === waiter.accountId &&
          ...
          slots.some(
            (slot) =>
              slot.purpose === waiter.purpose && !slot.child && !slot.stopping,
          ),
      );
      if (eligibleIndex >= 0) {
        ...
        clearTimeout(waiter.timer);
        try {
          waiter.resolvePromise(await startProcess(waiter.accountId, free));
        } catch (error) {
          waiter.rejectPromise(error);
        }
        continue;
      }
      ...
      const conflict = slots.find(
        (slot) =>
          ...
          !slot.leased &&
          !slot.stopping &&
          Date.now() >= slot.protectedUntil,
      );
      if (conflict) {
        stop(conflict, "切换运行模式");
        continue;
      }
...
const server = createServer(async (request, response) => {
  try {
    ...
    const coverMatch =
      request.method === "GET"
        ? url.pathname.match(
            /^\/trend-covers\/([a-zA-Z0-9_-]{8,100}\.(?:jpg|png|webp|avif))$/,
          )
        : null;
    if (coverMatch) {
    ...
    if (request.method === "GET" && url.pathname === "/health") {
      return json(response, 200, {
        ok: true,
        capacity: SLOT_PORTS.length,
        active: slots.filter((slot) => slot.purpose === "worker" && slot.child)
          .length,
        verificationActive: Boolean(
          slots.find((slot) => slot.purpose === "verification")?.child,
        ),
      });
    }
    if (request.method === "GET" && url.pathname === "/slots") {
      return json(response, 200, {
        pending: acquireQueue.length,
        slots: slots.map((slot) => ({
          port: slot.port,
          purpose: slot.purpose,
          accountId: slot.accountId,
          active: Boolean(slot.child),
          busy: slot.leased,
          stopping: slot.stopping,
          protectedUntil: slot.protectedUntil,
    ...
    if (request.method === "GET" && url.pathname === "/zhihu/auth-status") {
      ...
      return json(
        response,
        200,
        await zhihuBrowserAuthStatus(platformAccountRoot, appRoot, accountId),
      );
    }
    ...
    if (
      request.method === "POST" &&
      url.pathname === "/zhihu/browser-login-complete"
    ) {
      const payload = await body(request);
      return json(
        response,
        200,
        await zhihuBrowserAuthStatus(
          platformAccountRoot,
          appRoot,
          payload.accountId,
          true,
    ...
    if (request.method === "POST" && url.pathname === "/zhihu/publish") {
      const payload = await body(request);
      const result =
        payload.authMethod === "openapi"
          ? await publishZhihuArticle(platformAccountRoot, payload)
          : await publishZhihuArticleBrowser(
              platformAccountRoot,
              appRoot,
              payload,
            );
    ...
    if (request.method === "GET" && url.pathname === "/wechat/auth-status")
      return json(
        response,
        200,
        wechatAuthStatus(
    ...
    if (request.method === "POST" && url.pathname === "/wechat/credentials") {
      const payload = await body(request);
      return json(
        response,
        200,
    ...
    if (request.method === "POST" && url.pathname === "/wechat/test") {
      const payload = await body(request);
      return json(
        response,
        200,
        await testWechatCredentials(
          platformAccountRoot,
          payload.accountId,
          payload.appId,
          payload.appSecret,
    ...
    if (request.method === "POST" && url.pathname === "/wechat/publish") {
      const payload = await body(request);
      return json(
        response,
        200,
        await publishWechatDraft(platformAccountRoot, payload),
      );
    }
    if (request.method === "POST" && url.pathname === "/wechat/render") {
      const payload = await body(request);
      const theme = String(payload.theme || "default");
      if (!Object.hasOwn(wechatThemes, theme)) {
        return json(response, 400, { error: "不支持的公众号排版主题" });
      }
      return json(response, 200, {
        theme,
        themes: wechatThemes,
        html: renderWechatMarkdown(
          payload.markdown,
          theme,
          [],
          payload.style || {},
    ...
    if (request.method === "GET" && url.pathname === "/publish-asset") {
      ...
      if (
        !relative ||
        assetPath === publishAssetRoot ||
        !assetPath.startsWith(`${publishAssetRoot}/`) ||
        relative.includes("..") ||
    ...
    if (request.method === "GET" && url.pathname === "/ai/health") {
      return json(response, 200, {
        ok: publicAISettings().configured,
        ...publicAISettings(),
        rulesBundled: Object.values(promptFiles).every(existsSync),
      });
    }
    if (request.method === "POST" && url.pathname === "/ai/run") {
      ...
      if (
        ...
        payload.prompt.length > 250_000
      )
        return json(response, 400, { error: "分析任务内容不合法" });
    ...
    if (request.method === "POST" && url.pathname === "/publish-assets") {
      const payload = await body(request);
      if (!validAccountId(payload.claimId))
        return json(response, 400, { error: "内容任务标识不合法" });
      if (
        !Array.isArray(payload.files) ||
        !payload.files.length ||
        payload.files.length > 9
      )
        return json(response, 400, { error: "每篇笔记需要上传1-9张图片" });
      ...
      const paths = payload.files.map((file, index) => {
          ...
          mime === "image/png"
            ...
            : mime === "image/webp"
              ...
              : mime === "image/jpeg"
                ? "jpg"
                : "";
        if (!extension || typeof file.data !== "string")
          throw Object.assign(new Error("只支持 JPG、PNG 或 WebP 图片"), {
            status: 400,
          });
        ...
        if (
          ...
          throw Object.assign(new Error("图片为空或文件总大小超过限制"), {
            status: 400,
          });
    ...
    if (
      request.method === "POST" &&
      ["/acquire", "/release", "/discard", "/touch"].includes(url.pathname)
    ) {
      const payload = await body(request);
      if (!validAccountId(payload.accountId))
        return json(response, 400, { error: "账号标识不合法" });
      const purpose = payload.purpose ?? "worker";
      if (!validPurpose(purpose))
        return json(response, 400, { error: "运行用途不合法" });
      ...
      const slot = slots.find(
        (candidate) =>
          candidate.accountId === payload.accountId &&
          candidate.purpose === purpose,
      );
      if (url.pathname === "/discard") {
        if (slot) stop(slot, "异常重启");
            ...
            purpose === "verification"
              ? Date.now() +
                Math.max(
                  0,
                  Math.min(
                    Number(payload.holdMs) || 0,
                    VERIFICATION_IDLE_TIMEOUT_MS,
                  ),
                )
              : 0;
          if (payload.close === true && purpose === "verification")
            stop(slot, "验证完成");
    ...
    return json(response, error?.status || 500, {
      error: error instanceof Error ? error.message : "运行管理器异常",
    });
  }
});
...
const sweep = setInterval(() => {
  ...
  for (const slot of slots) {
    ...
    if (
      ...
      !slot.leased &&
      !slot.stopping &&
      now >= slot.protectedUntil &&
      now - slot.lastUsed > timeout
    )
      stop(slot, "空闲超时");
  }
}, 30_000);
...
server.listen(MANAGER_PORT, HOST, () =>
  process.stdout.write(
    `小红书运行管理器：http://${HOST}:${MANAGER_PORT}，采集槽位 ${SLOT_PORTS.join("、")}，可见验证槽位 ${VERIFICATION_PORT}\n`,
  ),
);
